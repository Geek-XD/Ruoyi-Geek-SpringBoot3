...
│       └── web                           // 前端控制
├── ruoyi-admin        // 后台服务
├── ruoyi-pay          // 支付场景（开发中-当前进度：核心功能已打通，业务需要自行开发）
│       └── common                         // 支付框架基础模块（功能可用，业务需要开发）
│       └── sqb	                           // 收钱吧支付模块（功能可用，业务需要开发）
│       └── wx	                           // 微信支付模块（功能可用，业务需要开发）
│       └── alipay	                   // 支付宝支付模块（功能可用，业务需要开发）
│       └── starter	                   // 支付场景启动器
├── ruoyi-auth        // 第三方认证场景（开发中-当前进度：核心功能已打通，业务需要自行开发）
